...
searchEnter.addEventListener("click", (event) => {
  ...
  switch (value) {
    case "character":
      ...
      break;
    case "seasonepisode":
      let season = document.getElementById("searchseason");
      let episode = document.getElementById("searchepisode");

      if (episode.value == "Episode") {
        window.location.hash = `s=${season.value}/`;
      } else {
        window.location.hash = `s=${season.value}/e=${episode.value}/`;
      }
